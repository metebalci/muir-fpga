...
**Nothing of the machine is built for this board.** This directory holds this
README and a pin file, `de25_nano_pins.tcl`. There is no top level, no flow,
no device tree and no image, and the machine has not been synthesized for its
part.
...
- the pins, which `de25_nano_pins.tcl` already carries, and an `.sdc`
  carrying the clocks and the exceptions;
...
Agilex 5 E-series only. Agilex 5 device support is installed, and Quartus
has fitted a test design that carries only this directory's pins. The machine
has not been synthesized for this part. Ubuntu 26.04 is not on Quartus's
...
The machine has not been synthesized for this part, so there is no fit. The
part is counted in adaptive logic modules and M20K blocks rather than in slices
and block RAM tiles, and a different tool maps the design onto it. So no ratio
