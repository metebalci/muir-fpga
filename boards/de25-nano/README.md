...
**The machine is built for this board with the processor's memory behind its
memory port, and it meets timing.** This directory holds the top level,
`cadr_de25.sv`, a Quartus flow under `quartus/`, the Linux side under
`linux/`, this README and a pin file, `de25_nano_pins.tcl`. `make de25`
builds the machine with nothing behind its memory port, as the Arty Z7-20's
default board is built; `DDR=1` adds the processor and its LPDDR4, and
`HDMI=1` adds the display output beside them. The processor's configuration
is `quartus/hps.tcl`, and the device tree, the Buildroot configuration and
the boot files are under `linux/`. `docs/toolchain.md` says how to build a
bitstream and load it.

**The board runs its band from its card and boots from power alone.** Its
QSPI flash carries this project's phase-1 bitstream, so it comes up with
nothing attached to it but power. Silicon has shown the machine's memory
cycles answered through the FPGA-to-SDRAM bridge, Linux on the processor with
the register faces on both processor-to-fabric bridges, a band loaded off the
card, and the machine's own screen on a monitor with a keyboard and a mouse
at the board. The serial line has carried nothing here, and the debug cable's
connector has crossed nothing. `docs/board.md` has each dated session and what
...
- a top level in this directory, the counterpart of
  `boards/arty-z7-20/cadr_arty.sv`. It is `cadr_de25.sv`, and it carries the
  processor, the register faces and the display output when the flow asks for
  them;
...
- the hard processor system's configuration, which `quartus/hps.tcl`
  describes and Platform Designer generates at every build, in place of the
  Zynq boards' `vivado/ps7_config.tcl` and `cadr_ps7.sv`;
- a device tree and a Linux image, which are
  `linux/buildroot/board/de25-nano/dts/intel/socfpga_agilex5_de25_nano_cadr.dts`
  and the Buildroot configuration beside it.
...
the machine's memory path. Here all three share `f2sdram` behind a
burst-level arbiter that gives the machine priority, and what that costs the
machine is measured rather than argued: "The memory board" below has the
figure.
...
port 1 and the display on port 2. On a board built without the display, that
port asks for nothing. It grants by burst, lets each master have one burst in
flight in each direction, and grants the others nothing new while the machine
is asking for a word or waiting for one. That hold is what bounds a machine
cycle --- what the machine waits for is what was already in flight when it
asked --- and priority by port alone does not do it: with one burst in flight
per master, a master waiting for its answer is not asking, so reversing the
priority moved not one number of the check. The measured worst is 27 ticks of
growth with both other ports streaming.
