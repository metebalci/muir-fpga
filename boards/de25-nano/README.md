...
| the debug probe and the build stamp | the build stamp is in the JTAG USERCODE register. There is no probe. |
...
configuration succeeded. The build stamp is in the USERCODE register, and
reading it back is not built: the JTAG server shows a design hash only for a
design with a debug hub, which this one has not got.
