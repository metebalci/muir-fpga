...
# **THE FAULT BITSTREAM, `FAULT=1`**: `boards/de25-nano/cadr_de25_fault.sv`,
# no machine and every lamp blinking, which U-Boot loads when the CADR's
# core image cannot be loaded.  It carries the memory board's processor
# system, generated by the same `hps.tcl`, because Linux runs on under it and
# its bridges must be answered; so it needs `DDR=1` and takes neither the
# display nor the probe.  Its checks are its own: no machine may be in it,
# the processor system must be, and its timing is `fault_sta.tcl`'s.  Into
# `build/de25-fault/`, with the fault mark in its build stamp.
fault=${FAULT:-0}
case $fault in
    0|1) ;;
    *) refuse "FAULT is '$fault'; it is 0, the CADR, or 1, the fault bitstream" ;;
esac
if [ "$fault" -eq 1 ]; then
    [ "$ddr" -eq 1 ] || refuse "FAULT=1 needs DDR=1: the fault bitstream keeps the processor system"
    [ "$hdmi" -eq 0 ] || refuse "FAULT=1 takes no HDMI=1: the fault bitstream has no display"
    out=build/de25-fault
fi
...
if [ "$fault" -eq 1 ]; then
    say "THIS IS THE FAULT BITSTREAM: no machine, every lamp blinking, into $out"
fi
if [ "$hdmi" -eq 1 ]; then
...
userid=$("$bin/quartus_sh" --tcl_eval source tools/build_stamp.tcl \; puts [lindex [build_stamp_of_tree $fault] 0] | tail -n 1)
if [ "$fault" -eq 1 ]; then
    "$bin/quartus_sh" --tcl_eval source tools/build_stamp.tcl \; puts [build_stamp_is_fault $userid] | tail -n 1 | grep -qx 1 \
        || refuse "the build stamp $userid does not carry the fault mark"
fi
...
if [ "$fault" -eq 1 ]; then
    # **THE FAULT BITSTREAM'S OWN CHECKS**: no machine, the processor system
    # and both bridges' default slaves.  The CADR's checks run to the `fi`
    # before the fit.
    if grep -q '; u_machine|' "$rpt"; then
        refuse "the fault bitstream has a machine in it; see $dir/$rpt"
    fi
    for ip in intel_agilex_5_soc emif_io96b_hps; do
        grep -q "; $ip *;[^;]*;[^;]*;[^;]*; u_hps|" "$rpt" \
            || refuse "synthesis lists no $ip under u_hps; see $dir/$rpt"
    done
    # Each default slave's two state machines, by instance: the report names
    # a parameter table once for instances that share one, so that is not
    # asked.
    for slave in u_h2f u_lw; do
        grep -q "State Machine - $slave|rst_r" "$rpt" \
            || refuse "synthesis has no default slave at $slave; see $dir/$rpt"
    done
...
say "6-sta-check"
if [ "$fault" -eq 1 ]; then
    sta_script=$here/fault_sta.tcl
else
    sta_script=$here/sta_check.tcl
fi
if ! env CADR_PIXEL_MHZ="$([ "$hdmi" -eq 1 ] && echo "$pixel_mhz" || echo 0)" \
        "$bin/quartus_sta" -t "$sta_script" > 6-sta-check.log 2>&1; then
...
# And the fault bitstream the other way: a fit of more than a few thousand
# ALMs, or any M20K block, is not a design without a machine.
if [ "$fault" -eq 1 ]; then
    if [ "$alms" -gt 3000 ] || [ "$m20k" -gt 0 ]; then
        refuse "the fault bitstream fits in $alms ALMs and $m20k M20K blocks, which is not a design without a machine; see $dir/$summary"
    fi
elif [ "$alms" -lt 1000 ] || [ "$m20k" -lt 40 ]; then
