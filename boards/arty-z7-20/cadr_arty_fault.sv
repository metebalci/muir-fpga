...
//
// The fault bitstream for the Arty Z7-20: no machine, every lamp blinking.
//
...
//
// **WHAT IT DOES.**  All six lamps blink together, two flashes a second:
// LD0 to LD3 green, and LD4 and LD5 red, with their green and blue pins dark.
...
// fabric masters memory: the three high-performance ports see no valid and
// take any response.
//
...
module cadr_arty_fault #(
    ...
    // Driven high to light, one pin a color.
    ...
    inout  wire  [7:0]     ja,
    // The HDMI connector at a steady level, as a board built without the
    // display leaves it: the pins are constrained `TMDS_33` and must be fed
    // through their differential buffers.
  ...
  // The three memory ports: no address, no data, and every response taken.
  logic hp0_aresetn, hp2_aresetn, hp3_aresetn;
  ...
  logic [1:0]  hp0_bresp, hp0_rresp, hp2_bresp, hp2_rresp, hp3_bresp, hp3_rresp;
  logic [63:0] hp0_rdata, hp2_rdata, hp3_rdata;
  ...
  cadr_ps7 u_ps7 (
      ...
      .hp3_awaddr(32'd0), .hp3_awlen(4'd0), .hp3_awsize(2'b11), .hp3_awburst(2'b01),
      .hp3_awvalid(1'b0), .hp3_awready(hp3_awready),
      .hp3_wdata(64'd0), .hp3_wstrb(8'd0), .hp3_wlast(1'b0), .hp3_wvalid(1'b0),
      .hp3_wready(hp3_wready),
      .hp3_bresp(hp3_bresp), .hp3_bvalid(hp3_bvalid), .hp3_bready(1'b1),
      .hp3_araddr(32'd0), .hp3_arlen(4'd0), .hp3_arsize(2'b11), .hp3_arburst(2'b01),
  ...
  OBUFDS u_hdmi_d0  (.I(1'b0), .O(hdmi_tx_d_p[0]), .OB(hdmi_tx_d_n[0]));
  OBUFDS u_hdmi_d1  (.I(1'b0), .O(hdmi_tx_d_p[1]), .OB(hdmi_tx_d_n[1]));
  OBUFDS u_hdmi_d2  (.I(1'b0), .O(hdmi_tx_d_p[2]), .OB(hdmi_tx_d_n[2]));
  OBUFDS u_hdmi_clk (.I(1'b0), .O(hdmi_tx_clk_p), .OB(hdmi_tx_clk_n));

  // What this design reads of nothing: the buttons, the switches, the
  ...
  assign unused = ^{btn, sw, mmcm_locked,
                    ...
                    hp0_bresp, hp0_rresp, hp2_bresp, hp2_rresp, hp3_bresp, hp3_rresp,
                    hp0_rdata, hp2_rdata, hp3_rdata};
