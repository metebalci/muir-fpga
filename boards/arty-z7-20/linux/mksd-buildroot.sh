...
# apart, and it is what keeps the menu honest.
{
  ...
  printf -- "#--time HHmm\r\n"
  printf "\r\n"
  printf "# Either line may stand alone and sets only its own field, leaving the\r\n"
  printf "# other as it is. The clock is saved to \`clock\` beside this file at a\r\n"
  printf "# clean shutdown and restored at the next boot, and the later of what\r\n"
  printf "# these two compose and what was restored is what the clock becomes.\r\n"
  printf "# So the clock never runs backwards: a date here that has already gone\r\n"
  printf "# by leaves the saved clock standing, and the boot log says which won.\r\n"
