...
#
# The mutation runner for cadr-serial: bugs `serial_test.c` has to catch.
#
...
#
# **ONLY THE FILES THE CHECK ACTUALLY BUILDS ARE COPIED.**  `cadr-serial.c` is
# the program's own `main` and is not linked into `serial_test`, so a record
...
#
#     mutate.py --list serial_mutations.txt --work DIR [--only NAME]
...
def apply(record, work, src, common):
    ...
    os.makedirs(os.path.join(here, "cadr-serial", "src"))
    os.makedirs(os.path.join(here, "cadr-common", "src", "cadr"))
    for f in CORE + HEADERS + ["serial_test.c"]:
...
def build_and_run(here, cc, cflags):
    src = os.path.join(here, "cadr-serial", "src")
    common = os.path.join(here, "cadr-common", "src")
    binary = os.path.join(here, "serial_test")
    cmd = ([cc] + cflags.split() + ["-I" + common, "-o", binary,
            os.path.join(src, "serial_test.c")]
    ...
    # A mutation that makes the endpoint wait for something that never comes
    # would otherwise hang the run rather than being reported.
