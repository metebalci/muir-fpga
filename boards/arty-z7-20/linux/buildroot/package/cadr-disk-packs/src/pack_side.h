...
//
// The pack side's register face, as `rtl/plumbing/cadr_disk_pack.sv` defines it and
...
// appearing in the bay, or going away.  `r_drive` in `rtl/plumbing/cadr_disk_pack.sv`
// is twenty-five bits and only 16:0 reach an output, so bits 24:17 are
// storage that reaches nothing, and `rtl/machine/cadr_disk_controller.sv` arms
