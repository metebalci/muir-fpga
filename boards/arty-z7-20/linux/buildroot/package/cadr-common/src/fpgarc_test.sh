...
EOF
	...
	#
	# \$CONSOLE_HDMI_MODE is the video mode this stubbed board's BITSTREAM
	# was built with, and it is the mode's name as \`cons_hdmi_mode_name\`
	# in the console program writes it, because the card's \`--hdmi-mode\`
	# word is looked for INSIDE the line the real console prints.  The
	# line's shape is the real one's too, since \`head -1\` is what the
	# script takes of it.
...
hdmi-mode)
	echo "hdmi: \${CONSOLE_HDMI_MODE:-1280x1024 at 60 Hz} --- the mode this" \\
	     "bitstream was built with; another mode is another bitstream"
	;;
*)
	case "\$*" in
...
# **THE DISPLAY OUTPUT'S MODE: --hdmi-mode ASKS AND NEVER SETS.**  A video mode
# is a pixel clock and a pixel clock comes from a clock manager whose dividers
# are fixed in the bitstream, so a bitstream carries one mode and the card's
# line is compared against what the fabric reports rather than written into it.
# The comparison is a SUBSTRING of the line `cadr-console hdmi-mode` prints, so
# the card's vocabulary is exactly the words that appear in one of the four
# names and in no other: `1280x1024`, `1400x1050`, `1080p30` and `1080p60`.
#
# **THE NAMES ARE READ OUT OF THE CONSOLE PROGRAM AND NOT COPIED HERE.**  A
# check carrying its own transcription of them would go on agreeing with itself
# while a board stopped matching, which is a check written to confirm.  So the
# stubbed fabric reports the names `console_face.c` really writes, on their own
# anchors, and a name that moves out from under the card's word fails here by
# name.  `console_test.c` holds the other half of it: each of the four words is
# inside exactly one of those names, and `1920x1080` is inside two.
FACE="$PKG/cadr-console/src/console_face.c"
hdmi_name() {
	sed -n "s/^[[:space:]]*case CONS_HDMI_$1:[[:space:]]*return \"\(.*\)\";\$/\1/p" \
	    "$FACE"
}
...
# What a run that matched looks like: the step says nothing at all.
hdmi_mode_silent() {
	if grep -q "cadr-display" "$WORK/out.mode"; then
		fail "$1; the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	else
		ok "$2"
	fi
...
case_head "the card's --hdmi-mode word names one mode, and the two 1080 rates are two words"
sandbox
if prepare cadr-disk-packs S80cadr-disk-packs; then
	# The anchors first, one line each, as every other rewrite in this
	# check is held: a name whose anchor stopped matching would give an
	# empty string, and an empty string is inside every line there is.
	_hbad=""
	for _hmode in 1280 1400 1920P30 1920P60; do
		_hcount=$(grep -c "^[[:space:]]*case CONS_HDMI_$_hmode:[[:space:]]*return \"" \
		     "$FACE" 2>/dev/null || true)
		[ "$_hcount" = 1 ] || _hbad="$_hbad CONS_HDMI_$_hmode($_hcount)"
	done
	if [ -z "$_hbad" ]; then
		ok "each mode's name is at its anchor in console_face.c exactly once"
	else
		fail "the anchors have rotted against console_face.c, matching:$_hbad"
	fi
	N1280=$(hdmi_name 1280)
	N1400=$(hdmi_name 1400)
	N30=$(hdmi_name 1920P30)
	N60=$(hdmi_name 1920P60)
	_hnames=$(printf '%s\n%s\n%s\n%s\n' "$N1280" "$N1400" "$N30" "$N60")
	if [ "$(printf '%s\n' "$_hnames" | grep -c .)" = 4 ] &&
	   [ "$(printf '%s\n' "$_hnames" | sort -u | grep -c .)" = 4 ]; then
		ok "the four modes' names were read out of console_face.c, and all four differ"
	else
		fail "the four modes' names could not be read out of console_face.c;" \
		     "what came back was: $_hnames"
	fi
	# **THE WHOLE POINT OF THE TWO WORDS IS THIS PAIR.**  Both of these
	# bitstreams are 1920x1080, so a card that said the resolution alone
	# would be taken on either one and say nothing on the wrong one.
	hdmi_mode_run 1080p30 "$N30"
	hdmi_mode_silent "a card asking for 1080p30 on a 1080p30 bitstream was told it was not" \
	                 "a card asking for 1080p30 on the 1080p30 bitstream is told nothing"
	if grep -qx "hdmi-mode" "$WORK/console.calls"; then
		ok "and the fabric was asked which mode it carries"
	else
		fail "the fabric was never asked; it was told: $(cat "$WORK/console.calls")"
	fi
	hdmi_mode_run 1080p30 "$N60"
	if grep -q "cadr-display: --hdmi-mode 1080p30, and this bitstream is not it" \
	   "$WORK/out.mode"; then
		ok "and the same card on the 1080p60 bitstream is told which bitstream it wants"
	else
		fail "a card asking for 1080p30 on the 1080p60 bitstream was not told so;" \
		     "the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	# **AND THE MIRROR**, because a script that matched nothing at all would
	# pass the refusing half above and fail nothing.
	hdmi_mode_run 1080p60 "$N60"
	hdmi_mode_silent "a card asking for 1080p60 on a 1080p60 bitstream was told it was not" \
	                 "a card asking for 1080p60 on the 1080p60 bitstream is told nothing"
	hdmi_mode_run 1080p60 "$N30"
	if grep -q "cadr-display: --hdmi-mode 1080p60, and this bitstream is not it" \
	   "$WORK/out.mode"; then
		ok "and the same card on the 1080p30 bitstream is told which bitstream it wants"
	else
		fail "a card asking for 1080p60 on the 1080p30 bitstream was not told so;" \
		     "the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
fi

# **AND THE BARE `1920x1080` IS REFUSED BY NAME RATHER THAN RESOLVED.**  It is
# inside both 1080 names, so on a board of either of them the comparison would
# match and this step would say nothing --- a setting somebody wrote down and
# did not get, which is the failure the line exists to prevent.  Picking one of
# the two silently would be that same failure under a new spelling, so the word
# is refused, the refusal names both words it could have meant, and it does not
# depend on reaching a console.
case_head "the bare 1920x1080 is refused by name, whichever bitstream is loaded"
sandbox
if prepare cadr-disk-packs S80cadr-disk-packs; then
	N30=$(hdmi_name 1920P30)
	N60=$(hdmi_name 1920P60)
	hdmi_mode_run 1920x1080 "$N60"
	if grep -q "cadr-display: --hdmi-mode 1920x1080 names two modes" "$WORK/out.mode"; then
		ok "a card saying 1920x1080 on the 1080p60 bitstream is refused by name"
	else
		fail "a card saying 1920x1080 was not refused; the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	if grep -q "1080p30" "$WORK/out.mode" && grep -q "1080p60" "$WORK/out.mode"; then
		ok "and the line names both words it could have meant"
	else
		fail "the refusal does not name both words; the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	if grep -q "and this bitstream is not it" "$WORK/out.mode"; then
		fail "the ambiguous word was compared against the bitstream as well"
	else
		ok "and it was not compared against the bitstream, so nothing was resolved"
	fi
	if grep -qx "hdmi-mode" "$WORK/console.calls"; then
		fail "the fabric was asked about a word that names two modes:" \
		     "$(cat "$WORK/console.calls")"
	else
		ok "and the fabric was not asked about it"
	fi
	# **AND A BOARD WITH NO CONSOLE AT ALL IS REFUSED TOO**, which is the
	# whole of "the refusal does not rest on reaching the fabric", measured
	# rather than inferred from the leg above.  The step has a guard for a
	# board whose image carries no `cadr-console`, and a refusal that sat
	# beyond that guard would be a card's word judged only where a fabric
	# could be asked.  Nothing on this host answers to that name either, so
	# moving the stub aside is a board that really has none.
	mv "$WORK/bin/cadr-console" "$WORK/bin/cadr-console.away"
	hdmi_mode_run 1920x1080 "$N60"
	mv "$WORK/bin/cadr-console.away" "$WORK/bin/cadr-console"
	if grep -q "cadr-display: --hdmi-mode 1920x1080 names two modes" "$WORK/out.mode"; then
		ok "and a board with no cadr-console at all still gets the refusal"
	else
		fail "a board with no cadr-console was not told its card's word names two" \
		     "modes; the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	# **THE OTHER BITSTREAM, WHICH IS THE CONTROL.**  The word is ambiguous
	# on the card and not on the board, so the refusal cannot depend on
	# which mode is loaded; a step that refused only the 60 Hz board would
	# pass the half above on its own.
	hdmi_mode_run 1920x1080 "$N30"
	if grep -q "cadr-display: --hdmi-mode 1920x1080 names two modes" "$WORK/out.mode"; then
		ok "and the same card on the 1080p30 bitstream gets the same refusal"
	else
		fail "the refusal depends on which bitstream is loaded; the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	if [ -s "$WORK/daemon.calls" ]; then
		ok "and the boot goes on: the pack program was started anyway"
...
# **THE OTHER TWO WORDS NEED NO RATE AND ARE UNCHANGED.**  Nothing about
# 1280x1024 or 1400x1050 is ambiguous, so each still names its own mode and
# nothing else, and a card that says nothing about the mode asks nothing.
case_head "1280x1024 and 1400x1050 each name their own mode, and silence asks nothing"
sandbox
if prepare cadr-disk-packs S80cadr-disk-packs; then
	N1280=$(hdmi_name 1280)
	N1400=$(hdmi_name 1400)
	N60=$(hdmi_name 1920P60)
	hdmi_mode_run 1280x1024 "$N1280"
	hdmi_mode_silent "a card asking for 1280x1024 on a 1280x1024 bitstream was told it was not" \
	                 "a card asking for 1280x1024 on its own bitstream is told nothing"
	hdmi_mode_run 1280x1024 "$N1400"
	if grep -q "cadr-display: --hdmi-mode 1280x1024, and this bitstream is not it" \
	   "$WORK/out.mode"; then
		ok "and on the 1400x1050 bitstream it is told which bitstream it wants"
	else
		fail "a card asking for 1280x1024 on the 1400x1050 bitstream was not told so;" \
		     "the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	hdmi_mode_run 1400x1050 "$N1400"
	hdmi_mode_silent "a card asking for 1400x1050 on a 1400x1050 bitstream was told it was not" \
	                 "a card asking for 1400x1050 on its own bitstream is told nothing"
	hdmi_mode_run 1400x1050 "$N60"
	if grep -q "cadr-display: --hdmi-mode 1400x1050, and this bitstream is not it" \
	   "$WORK/out.mode"; then
		ok "and on the 1080p60 bitstream it is told which bitstream it wants"
	else
		fail "a card asking for 1400x1050 on the 1080p60 bitstream was not told so;" \
		     "the script says:"
		sed 's/^/        /' "$WORK/out.mode"
	fi
	...
	run_script S80cadr-disk-packs
	if grep -qx "hdmi-mode" "$WORK/console.calls"; then
		fail "the fabric was asked about the mode by a card that says nothing:" \
		     "$(cat "$WORK/console.calls")"
	else
		ok "a card that says nothing about the mode does not ask about it"
	fi
fi
