...
# boards/arty-z7-20/vivado/gen_ps7_init.tcl, which applies them to a bare `processing_system7`
...
# gives a BYTE-IDENTICAL routine, and adding `S_AXI_HP1` at 64 bits on top
# changes nothing either.  Only 32-bit HP0 differs, by exactly two writes per
# silicon revision --- `0xF8008000` and `0xF8008014`, the AFI0 read and write
# channel controls, both annotated `n32BitEn = 1`, and both in
# `ps7_post_config` rather than in `ps7_init`.  So the whole of this file is
# board configuration, none of it is ours, and every property we changed would
# be a configuration nobody has validated on this board.  We use no FCLK --- the
...
# `boards/arty-z7-20/cadr_arty.sv`, 100 MHz since 2026-09-11, and changing a
...
# holds only `.gitmodules` and a README reading "Root commit".  The board's
# project lives on a per-board branch of a submodule:
#
#     repository  github.com/Digilent/Arty-Z7-HW
#     branch      20/OOB/master
#     commit      05c27fc83df637dc314d2d21fd9b8f36633ab38b
#     file        src/bd/design_1.tcl
#     lines       282-840
#
# The 559 lines below that line range are VERBATIM, so a reader can tell
# whether the pointer has rotted again without trusting this header:
#
#     git clone -b 20/OOB/master https://github.com/Digilent/Arty-Z7-HW
...
# has.  The disk's pack side, `rtl/plumbing/cadr_disk_pack.sv`, is the master on it;
...
# `rtl/plumbing/cadr_console.sv` is the slave on that port, presenting the machine's
...
# PetaLinux BSP (`Petalinux-Arty-Z7-20-2017.4-1.bsp`, sha256 a83dbe29...,
# `project-spec/hw-description/ps7_init.tcl`, generated 2018 by a 2017.4 tool)
# on every DDR, PLL, MIO and post-config write --- character for character.
# They differ in six writes and four delays per silicon revision, all of them
# in `ps7_clock_init` and `ps7_peripherals_init`, and all of them explained by
# the two designs' own configurations: the 2017.4 base design enabled SPI0 and
# four FCLKs where this one has two FCLKs and no SPI.  Measured 2026-09-09,
# reported in the commit that added this file.
#
# And the routine this generates is byte-identical, apart from the CRLF the
# zip left on it, to the `ps7_init.tcl` Digilent themselves ship inside
# `hw_handoff/design_1_wrapper.xsa` at the commit named above.  Two tools eight
# releases apart, the same file.
#
# ONE THING VIVADO SAYS THAT IS NOT A FAULT.  Applying these draws two
# `CRITICAL WARNING [PSU-1]` and `[PSU-2]` about negative
# `PCW_UIPARAM_DDR_DQS_TO_CLK_DELAY_2` and `_3`.  Those are byte lanes 2 and 3,
# and `PCW_UIPARAM_DDR_BUS_WIDTH` is `16 Bit` --- one MT41K256M16, 4 Gbit, the
# board's 512 MB on a 16-bit bus.  The lanes the warning names are not
# populated.
