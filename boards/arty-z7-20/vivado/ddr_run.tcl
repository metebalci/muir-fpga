...
#         timeout 900 ~/Xilinx/2026.1/Vivado/bin/xsdb boards/arty-z7-20/vivado/ddr_run.tcl
#
# Run from the repository root.  `boards/arty-z7-20/vivado/ddr_check.tcl` is step one and
# `boards/arty-z7-20/vivado/prove_write.tcl` step two; their headers carry the argument for the
...
# `rtl/plumbing/cadr_mem_count.sv` keeps four sixteen-bit counters: what the machine
...
# ------------------------------------------------------------------ the map
# rtl/plumbing/cadr_ddr_map.sv's MAIN_BASE, and the page the parity loop walks.  The
...
for {set i 0} {$i < $BLOCK_WORDS} {incr i} {
    ...
    if {$p == 0 || $p == 0xFFFFFFFE} {
        ...
        say "FAILED   DDR reads on this board.  boards/arty-z7-20/vivado/ddr_check.tcl measured"
...
if {$probe != 0x5A5AA5A5} {
    ...
    say "FAILED   boards/arty-z7-20/vivado/ddr_check.tcl is the step that isolates this."
    bye 1
}
...
# `rtl/plumbing/cadr_mem_count.sv` packs them.  Returns the raw pair followed by the
...
# These return prose and not numbers; `boards/arty-z7-20/vivado/prove_write.tcl`'s header has
...
if {![regexp {DONE \(Bits \[5\]\): 1} $irstat]} {
    ...
    say "FAILED   the bit boards/arty-z7-20/vivado/program.tcl reads as REGISTER.IR.BIT5_DONE,"
...
if {![marked [lindex $first 0]] || ![marked [lindex $first 1]]} {
    ...
    if {[lindex $first 0] == 0xFFFFFFFF && [lindex $first 1] == 0xFFFFFFFF} {
        ...
        say "FAILED   rtl/plumbing/cadr_mem_count.sv is inside the DDR generate, so a"
