...
#
# Write `boards/arty-z7-20/cadr_ps7.sv` and `tb/cadr_ps7_stub.sv` out of Xilinx's own
...
RTL = os.path.join(REPO, "boards", "arty-z7-20", "cadr_ps7.sv")
...
#
# `SAXIHP0ACLK` is an INPUT, driven by our own 100 MHz: the fabric clocks the
...
EXPOSED = [
    ...
    # system's GPIO, and it is how `rtl/plumbing/cadr_mem_count.sv`'s tally is read.
    ...
    # `rtl/plumbing/cadr_disk_pack.sv` is the master on it.
    ...
    # it.  `MAXIGP0ACLK` is an input, driven by our 100 MHz like the HP
    ...
    # reason: `rtl/plumbing/cadr_console.sv` is the slave on it, presenting the
    ...
    # GP0 would mean a decode in front of `rtl/plumbing/cadr_disk_pack.sv`'s face,
    ...
    # `boards/arty-z7-20/cadr_arty.sv` are one change.
    ...
    # system: bit 0 is the disk's, `rtl/plumbing/cadr_disk_pack.sv`'s `irq` --- a
...
def wrapper(ports):
    ...
    out = [SPDX, """//
// GENERATED by boards/arty-z7-20/vivado/gen_ps7.py from $XILINX_VIVADO's own PS7.v.
...
// Do not edit; run `make ps7`.
//
// `PS7` as an empty shell, so that Verilator can elaborate `boards/arty-z7-20/cadr_ps7.sv`
// and lint the top level that instantiates it.
//
// **THIS FILE MUST NEVER MOVE TO `rtl/`.**  `boards/arty-z7-20/vivado/fit.tcl` and
// `boards/arty-z7-20/vivado/bitstream.tcl` both read `[glob rtl/*/*.sv rtl/*/*/*.sv boards/arty-z7-20/*.sv]`, so a stub here would be
        ...
        for path, text in sorted(want.items()):
            ...
            if have != text:
                die("%s is not what the generator writes today: run `make ps7`"
                    % os.path.relpath(path, REPO))
        print("ok: boards/arty-z7-20/cadr_ps7.sv and tb/cadr_ps7_stub.sv are current "
