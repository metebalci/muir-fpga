...
#
# Write the Cora Z7-07S's Zynq start-up routine, out of Digilent's own board
# configuration for this board.
#
#     vivado -mode batch -source boards/cora-z7-07s/vivado/gen_ps7_init.tcl
...
#
# WHAT IT GENERATES IS NOT CHECKED BY EYE. boards/cora-z7-07s/vivado/ps7_ops.py extracts the
# ordered register operations from the routine and compares them against the
# committed boards/cora-z7-07s/vivado/ps7_init.ops, so a change in Digilent's configuration or in
...
# not `cadr_ps7` --- that name is the fabric wrapper `boards/cora-z7-07s/vivado/gen_ps7.py`
# writes, and two different things must not share one name.

set part   [expr {[info exists ::env(PART)]   ? $::env(PART)   : "xc7z007sclg400-1"}]
set outdir [expr {[info exists ::env(OUTDIR)] ? $::env(OUTDIR) : "build/ps7-cora"}]

set ipdir $outdir/ip
file delete -force $ipdir
file mkdir $ipdir

source boards/cora-z7-07s/vivado/ps7_config.tcl
if {![info exists ps7_config]} {
    puts "gen_ps7_init: boards/cora-z7-07s/vivado/ps7_config.tcl set no ps7_config"
    exit 1
}
