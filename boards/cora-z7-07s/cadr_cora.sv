...
//
// The machine on a Cora Z7-07S: a top level with real pins.
//
// **THIS IS THE ARTY Z7-20's TOP LEVEL WITH THIS BOARD'S PINS ON IT, AND
// NOTHING IN `rtl/` CHANGES BETWEEN THEM.**  A second Zynq board is a top
// level, a pin file, a processing-system configuration and a device tree; the
// machine does not know what part it is on.  So this file is
// `boards/arty-z7-20/cadr_arty.sv` with five differences, each of them a fact
// of the board rather than a decision about the machine:
//
...
//     with, so the fabric always comes up with `RUN` preset and the hold is
//     the card's `fpgarc` flag alone --- which is a Linux init step writing
//     the console's halt, and works here exactly as it does there.  What is
//     lost is the hold on a board with no Linux on it yet, which is a
//     bring-up convenience and not a capability of the machine.
//   - **NO HDMI, SO NO DISPLAY OUTPUT AND NO `S_AXI_HP3`.**  Digilent's file
//     has no HDMI section for this board at all, where the Arty Z7-20's has
//     both a receiver and a transmitter.  There is therefore no `HDMI`
//     parameter here, no differential pair in the port list, and the Cora's
//     `cadr_ps7.sv` does not bring `S_AXI_HP3` out.  The CADR's screen on this
//     board is `cadr-terminal`'s RFB server over the network, which is what
//     serves it on the other board as well.
//   - **NO USB INPUT.**  A keyboard and mouse reach the machine through
//     `cadr-terminal` from a viewer.  That program is unchanged; what is not
//     installed on this board is `cadr-usb-input`, and nothing in the fabric
//     differs for it --- the input face is on `M_AXI_GP0` here as it is
//     there.
//
// **AND ONE THING THAT IS NOT A DIFFERENCE IN THIS FILE BUT MATTERS BESIDE
// IT: THE XC7Z007S HAS ONE CORTEX-A9 WHERE THE XC7Z020 HAS TWO.**  muir on
// the board was measured to cost the fabric machine nothing with both cores
// saturated.  On one core it shares that core with `cadr-disk-packs`, which
// is on the machine's critical path, and nobody has measured what that costs.
//
// **NOTHING IN THIS FILE HAS BEEN ON SILICON.**  No Cora Z7-07S has ever been
// programmed with a bitstream from this repository.  What is measured is the
// fit: `boards/cora-z7-07s/README.md` carries it with the commit.
//
// Everything below this point is the Arty's file.  What follows is its own
// header, which is the argument for the tick, for the fold and for the clock,
// and is as true of this board as of that one.
//
// ---------------------------------------------------------------------------
//
// The machine on a board: a top level with real pins.
//
...
//
// **The board's clock is 125 MHz and the machine's is 100.**  The Cora
// Z7-07S's PL system clock is on the same pin at the same frequency as the
// Arty Z7-20's, so the recipe carries over with nothing changed: the 100 MHz
...
// in nanoseconds**, which is what lets `boards/arty-z7-20/vivado/tick.tcl` ---
// read by this board's flows too, because how long a tick is is not a
// property of a part --- read the number back out of this file and hand it to
// `create_clock` and to the constraint assertions --- so no constraint can describe a different machine
...
// `boards/cora-z7-07s/vivado/bitstream.tcl` checks the utilization against what the design is
...
// LUTs, the same registers, the same 28 block RAM tiles `boards/cora-z7-07s/vivado/bitstream.tcl`
// measures.  Setting it instantiates `cadr_probe.sv`, which records one
// sample a microcycle and hands it back over JTAG; `boards/cora-z7-07s/vivado/probe.tcl` builds
...
module cadr_cora #(
    ...
    // the board has, and there are no slide switches at all --- see the note
  ...
  MMCME2_BASE #(
      ...
      // is 100 MHz.  `boards/arty-z7-20/vivado/tick.tcl`, which this board's flows read too,
      // parses these four
  ...
  // wiring is `make build/cora.pass`, which lints every board configuration
  ...
  // it is the one instrument `boards/cora-z7-07s/vivado/ddr_run.tcl` reads with nobody at the
  ...
  // ------------------------------ NO SW0: THE MACHINE ALWAYS AUTO-BOOTS
  //
  // **THIS BOARD HAS NO SLIDE SWITCH, SO `no_auto_boot` IS TIED LOW AND THE
  // FABRIC ALWAYS COMES UP WITH `RUN` PRESET.**  On the Arty Z7-20 SW0 holds
  // the machine at power-on as a CADR is held when its power comes on with
  // nobody at it --- `RUN` and `SRUN` clear, the boot PROM never run, any
  // `-BOOT` source starting it.  Digilent's master pin file for this board has
  // two buttons and no switches at all, so there is no pin to read.
  //
  // **WHAT IS LOST IS A BRING-UP CONVENIENCE AND NOT A CAPABILITY.**  The
  // card's `fpgarc` carries `--no-auto-boot` and the disk pack program's init
  // script holds the machine through the console when it is set, which works
  // here exactly as it does there --- the console is on `M_AXI_GP1` on both
  // boards.  What a switch buys over the flag is a hold on a board whose Linux
  // is not up yet, and the boot PROM runs for a few hundred milliseconds
  // before Linux can halt it either way.  So the hold on this board is the
  // flag alone, and the console's two report words say so: both read zero,
  // which is the truth about a board with no switch on it.
  //
  // Tied at the instantiation rather than through a named constant, so that
  // `no_auto_boot` has exactly one driver and a reader looking for a switch
  // finds this note instead of a wire.
  ...
  // machine behind the memory port and so none of the three to report.
  ...
  // UNUSEDSIGNAL and `build/cora.pass` says so.
  logic [2:0] lamp1;
  ...
  // The heartbeat's counter. Declared here and counted up in the lamps
  // where its comment is, because a `PROVE` board's LD1 blinks off it and the
  ...
  cadr_machine #(
      ...
      // There is no switch on this board, so the machine always comes out
      // of reset with `RUN` and `SRUN` preset; the note where the Arty
      // Z7-20 reads SW0 says what that costs and what carries the hold
      // instead.  `-BOOT` comes back out for the error lamp to be cleared
      // by.
  ...
  if (PORT != 0) begin : g_ddr
    ...
    if (PROVE == 0) begin : g_machine_drives
      ...
      // that is what `boards/cora-z7-07s/vivado/prove_read.tcl` asserts. This lamp costs
      // nothing and is for whoever happens to be at the board.
      assign lamp1 = {!prove_has_run && (port_rst_sync[2] || tick[24]),
    ...
    // the arrangement; `build/cora.pass` holds the wiring, and an
    ...
    cadr_console u_console (
        ...
        // SW0's two report words.  There is no switch on this board, so
        // both read zero for ever and `cadr-console status` says the machine
        // was not held by one --- which is the truth here, and is why they
  ...
  if (PROBE_DEPTH > 0) begin : g_probe
    ...
    // part --- which `boards/cora-z7-07s/vivado/probe.tcl` selects by name and by code.
    //
  ...
  //
  // `cadr_cora.xdc` already false-paths `witness_reg` by name and goes on
  ...
  // **AND THE HEARTBEAT HAS NO LAMP ON THIS BOARD.**  On the Arty Z7-20
  // `tick[25]` is LD1 and answers "is this thing clocked at all", which is the
  // first question on a dark board.  Two lamps cannot carry it as well as the
  // machine's own state, so it is not carried, and what that costs is real and
  // is stated rather than worked around: on this board a board that was never
  // programmed, a board whose MMCM never locked and a machine that has stopped
  // all look the same, and telling them apart is a job for the console or for
  // the debugger.  The counter itself stays, because a `PROVE` board's LD1
  // blinks off it while it waits for the port.

  // ================================== THE TWO LAMPS ==========================
  //
  // **THIS BOARD HAS TWO RGB LEDS AND NOTHING ELSE, SO WHAT THEY SAY IS A
  // DECISION AND NOT A TRANSLATION OF THE ARTY Z7-20's SIX.**  The CADR's own
  // light panel carried a run lamp and an error lamp beside the boot button,
  // and BTN0 is that button here, so the two lamps are the two the panel had.
  //
  // **AND THEY ARE NAMED BY THE MACHINE'S OWN NETS, AS THE OTHER BOARD'S
  // ARE.**
  //
  //   LD0  MACHRUN          GREEN as a level: lit means the machine should be
  //                         running.  `MACHRUN` is `(SSTEP AND -SSDONE) OR
  //                         (SRUN AND -ERRHALT AND -WAIT AND -STATHALT)` at
  //                         OLORD1 1A15, so it drops during every memory
  //                         stall --- which makes the lamp's BRIGHTNESS the
  //                         fraction of time the machine computes rather than
  //                         waits.  Dim means it is thrashing.
  //   LD1  what it is doing RED for `ERRHALT`, BLUE for `-PROMDISABLE`, and
  //                         otherwise GREEN blinking with the microcycles.
  //                         Red wins over blue, blue over green.
  //
  ...
  //           would still hold a level high.  **OR A LEVEL, with
  //           `--no-blinking-leds`**: lit for a moment after every
  //           microcycle, so solid while the machine runs and dark shortly
  //           after it stops.  Red and blue do not change and
  ...
  //           clears `RUN` --- so stopping the machine to look at it leaves
  //           the lamp on whatever it was.  **Latched and cleared by any
  //           `-BOOT` press**, which is why `-BOOT` comes out of the machine:
  //           a board booted at the button, at the keyboard's chord or over
  //           the debug cable starts with a clean lamp and nothing out here
  //           has to know which of the three it was.
  //
  // So blue then green is a boot, green gone still is a machine somebody
  ...
  // from one that does not.  WHICH signal reaches its input is this file, and
  // this file is reached by `build/cora.pass`'s lint and by nothing else ---
  // so a second term ORed in here would be caught by nobody, and the reason it
  // is not there is this paragraph.  It is the same module and the same split
  // the Arty Z7-20 uses, which is what makes the two boards' red lamps the
  // same claim.
  //
  // **AND WHAT TWO LAMPS CANNOT SAY IS SAID HERE RATHER THAN LEFT TO BE
  // DISCOVERED.**  Two of the Arty's six meanings have no lamp on this board:
  // the fabric's own heartbeat, above, and disk activity, so a pause cannot be
  // told from the disk by looking.  Both signals are still outputs of
  // `cadr_machine` and still in the `witness` fold, so the fitter still sees
  // them; what is missing is a pin, not a wire.
  //
  // **AND ON A `PROVE` BOARD LD1 SAYS SOMETHING ELSE ENTIRELY** --- the
  ...
  assign {led1_r, led1_g, led1_b} = lamp1;
  if (PROVE == 0) begin : g_lamp_state
    ...
    // keeps for the `PROVE` arm's sake.
    /* verilator lint_off UNUSEDSIGNAL */
    logic unused_tick;
    assign unused_tick = ^tick;
    /* verilator lint_on UNUSEDSIGNAL */
  end

  // **LD0 IS REGISTERED, AND THAT IS NOT A CONVENIENCE.**
  //
  ...
  // to.
  assign led0_r = 1'b0;
  assign led0_g = machrun_lamp;
  assign led0_b = 1'b0;

  // **EVERY PIN THIS BOARD HAS IS USED**, which is why there is no fold of
  // unread inputs here where the Arty Z7-20 has one: both buttons drive
  // something and there are no switches.  The one signal with no reader is
  // the heartbeat counter, read in the lamp block above where its reason is.
