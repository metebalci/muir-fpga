...
every datapath signal on it, and everything that is a matter of *when*: bus
waits and hangs, arbitration, timeouts. This repository is that machine in
fabric, at the CADR's own 145 ns microcycle.

muir's netlists are read to **derive** things: the port list's directions, the
address decode's boundaries, and every constant that came off a drawing. But
nothing here is a netlist. Whether a gate-level implementation ever follows is
an open question, and it is not a promise this repository makes.

This is separate from muir because the toolchain is separate. muir stays the
reference, and everything here is checked against it.

## Target

The target is the Digilent **Arty Z7-20**, `XC7Z020-1CLG400C`. It has 53,200
...
the Trident bus timing and the Chaosnet cable are in fabric. RFB, the pack on
microSD, Chaosnet routing and the console are on the PS. Main memory is the
only seam that runs at machine speed, and it is the one with no software in it.
...
Trident turns 60 times a second with 17 blocks to the track, which is 1,020
blocks a second of 256 words each, so a streaming pack is 261,120 words a
second, just over a megabyte. Bandwidth was never the point. The point is that
on a port the PS masters, those are 261,120 stalled CPU stores a second,
against a 968 us deadline for each block. So fabric fetches the block out of
DDR itself, off the block's address written by Linux, and no CPU is in the
per-word path. The disk gets its own port rather than a share of `HP0`, so that
disk traffic adds no arbitration to the memory path. `M_AXI_GP0` and AXI4-Lite
carry what is left: the disk's registers, the Chaosnet buffers and the console.
`M_AXI_GP1` carries the debug cable, which has a section of its own below. The
Zynq-7000 PS-PL ports are **AXI3**, not AXI4. The logic here is AXI4, and
Vivado's converter bridges the two.
...
Of the 92, 15 are inputs and 29 are outputs. The other 48, exactly `MEM<31:0>`
and `SPY<15:0>`, are driven from both ends. Fabric has no bus to fight over, so
...
collision is an error. `rtl/machine/cadr_cables.map` holds every identifier against the name MIT wrote.
...
levels as registers. It drives `DBGIN` as machine A's `DBGOUT` page would, or
as the PDP-11 once did, with the phase generator held at a microcycle boundary
while muir computes its side. Machine-time stays exact and only wall-clock
stretches. muir already runs the lashup, so the debugger is software that works
before the fabric it is pointed at does.

A second board is the same cable on two Pmods, one clock and seven data each
way. That is 22 bits out and 19 back, four beats and three, tens of nanoseconds
against those 11.05 us.

**The console is not this, and the difference is worth keeping.** It masters
the machine's own Unibus over `M_AXI_GP0` to reach SPY. No CADR had that path,
...
Memory is shared with the Linux side, so it is settled early in `rtl/plumbing/cadr_ddr_map.sv` and
**reserved at the size the machine could one day want**. That is a quarter of
the board's 512 MB, and Linux keeps 384.
...
fidelity level. Here the two coincide.
