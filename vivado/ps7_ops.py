...
# checked.
#
#     python3 vivado/ps7_ops.py            # regenerate vivado/ps7_init.ops
#     python3 vivado/ps7_ops.py --check    # is it what the flow writes today?
#     python3 vivado/ps7_ops.py --from F   # extract from an existing routine
#
# WHY THE OPS AND NOT THE ROUTINE.  `vivado/gen_ps7_init.tcl` writes an 853
...
# the proc it is in.  Committed as vivado/ps7_init.ops, checked by
# `make current`.
#
...
# vivado/ps7_config.tcl agrees with the one in Digilent's 2017.4 PetaLinux BSP
...
# `vivado/gen_ps7.py`'s precedent: CI has no Vivado, the committed file is the
...
GEN = os.path.join("vivado", "gen_ps7_init.tcl")
...
def rendered(ops):
    ...
    """
    head = [SPDX, "#",
            "# GENERATED by vivado/ps7_ops.py from the ps7_init.tcl that",
            "# vivado/gen_ps7_init.tcl writes out of vivado/ps7_config.tcl.",
            ...
            " vivado/ps7_config.tcl for where the",
...
def committed():
    try:
        ...
        die("vivado/ps7_init.ops is missing: run `make ps7-init` and commit it")
...
def main():
    ...
    if args.check:
        ...
        if have != ops:
            sys.stderr.write(
                "ps7_ops: vivado/ps7_init.ops is not what the flow writes "
                "today\n")
            report(have, ops)
            die("run `make ps7-init` and commit, or find out why the routine "
                "moved")
        print("ps7_ops: ok: vivado/ps7_init.ops is current (%d operations, "
              "%d procs)" % (ops_count(ops), procs_count(ops)))
        return 0

    with open(OPS, "w") as f:
        f.write(rendered(ops))
    print("ps7_ops: wrote vivado/ps7_init.ops (%d operations, %d procs)"
          % (ops_count(ops), procs_count(ops)))
    return 0
