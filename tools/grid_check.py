...
STATEMENT = re.compile(
    ...
    r"|(assert_multicycle_applied|assert_instance_timing)\s+\$tick\s+(\d+))")
TAG = re.compile(
    r"^\s*#\s*(?:grid:\s*(\d+)\s*ns(?:\s*\(shared with\s+([\d\s,and]+?)\s*ns\))?"
    r"|(board ticks))\s*$")
...
def check_constraints(root, grid):
    ...
    for path in sorted(files):
        ...
        for i, line in enumerate(lines):
            ...
            if kind == "hold":
                if count != want - 1:
                    fail(f"{where}: a hold of {count} beside {ns} ns, which is {want} ticks at "
                         f"the {grid} ns grid: the hold is one less, {want - 1}")
            elif count != want:
                fail(f"{where}: {count} ticks for {ns} ns, which is {want} at the {grid} ns "
                     f"grid of rtl/machine/cadr_tick_pkg.sv")
            shared = sorted({int(x) for x in re.findall(r"\d+", t.group(2) or "")})
            found.append((where, kind, ns, count, shared))
    ...
    for where, kind, ns, count, shared in grid_tagged:
        if kind != "assert_multicycle_applied":
            continue
        others = sorted({o for (o, c) in constrained if c == count and o != ns})
        if others != shared:
