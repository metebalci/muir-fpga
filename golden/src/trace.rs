...
impl Trace {
    ...
    /// `-MEMACK`'s instant for the cycle in flight, and `-MEMGRANT`: muir's
    /// own, `Rtl::bus_ack_at` and `Rtl::bus_granted`, which are the CADR's
    /// bus interface or QUUX's memory port, whichever the machine has.
