...
**`DDR=1` builds the memory board**, the machine with the processor, its
LPDDR4 and its FPGA-to-SDRAM bridge behind its memory port, into
`build/de25-ddr/`. The processor system is described in
`boards/de25-nano/quartus/hps.tcl` and generated by `qsys-script` and
`qsys-generate` at every build, as the PLL is; nothing generated is
committed. `DE25_DDR_MHZ` is the memory's speed, 1066.667 by default, which
runs on either revision of the board, or 1333.333 on a rev B board.

    make de25 DDR=1                      # into build/de25-ddr/
    make de25 DDR=1 DE25_HPS_BOOT=fpga-first DE25_SPL_HEX=<path>

**A part with a processor in it is not configured by a bare `.sof`.** With
`DE25_SPL_HEX` naming the first-stage loader the Linux side builds, the flow
writes the file the board needs: one `.sof` with that loader in it for a
board loaded over JTAG, which must be built `fpga-first`, or the phase-1 and
core bitstreams for a board that boots from its flash. `make de25-program
DDR=1` loads the first of those.
