...
// and all sixteen read.  `rtl/cadr_console.sv` is a second master on that
...
//   2. IDENT.  Word 0 reads "CONS".  "NONE" is `rtl/cadr_gp0_default.sv`'s
...
//            `rtl/cadr_memory_path.sv` answers only `0o766xxx` on its Unibus
...
//            machine's reserved region, at `rtl/cadr_ddr_map.sv`'s base and
...
static int in_range(uint32_t phys)
{
	...
	say("word %o (%u) is past the %u words this machine can address --- its physical address is 22 bits and "
	    "the top four 64K slots are the display, the disk controller and the Unibus (rtl/cadr_ddr_map.sv)",
