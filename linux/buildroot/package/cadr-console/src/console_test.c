...
//     console_test
//
// **A MODEL AND NOT THE RTL.**  What follows `rtl/cadr_console.sv` at this
...
static uint32_t model_read(struct console *c, unsigned word)
{
	...
	default: return CONS_UNMAPPED;	/* words 6-15 */
	}
}
...
static void check_guard(void)
{
	...
	CHECK(CADR_TALLY_MARK == 0x00008000u && CADR_TALLY_MASK == 0x80008000u,
	      "the marker pattern has moved from what rtl/cadr_mem_count.sv writes");
}
...
int main(void)
{
	...
	printf("ok: the console program drives the register face rtl/cadr_console.sv defines, and\n"
	       "    says what it found\n"
