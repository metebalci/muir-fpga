...
// cannot see either and neither can `rtl/cadr_console.sv` through
...
// instantiated by `rtl/cadr_machine.sv`, where `vma`, `q` and `mclk` all are,
// and by `tb/cadr_console_harness.sv` --- the same module and not a copy of
// it, which is the rule `rtl/cadr_console_bus.sv` was extracted for: two
// descriptions of one thing drift and the check then holds the copy.
//
// Being inside `cadr_machine` is not a tidiness: `rtl/cadr_machine.xdc` is
