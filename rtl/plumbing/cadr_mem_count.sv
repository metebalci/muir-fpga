...
module cadr_mem_count (
    input  var logic clk,
    // THE MACHINE'S RESET AND NOT THE PORT'S.  `boards/arty-z7-20/cadr_arty.sv` holds the
    ...
    // and `rtl/plumbing/cadr_prove.sv`'s on a `PROVE` one: a level held up until the
