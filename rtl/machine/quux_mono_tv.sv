...
//                 leftmost, as on the CADR's TV.  The buffer is on the
//                 memory bus with main memory (contract Q7): the memory
//                 port's cache holds its words and writes them through to
//                 DDR at the display's base, where the scanout reads them.
//                 This module says only that the cycle is the buffer's
//                 (`fb_sel`), which is how the port tells it from a
//                 register.  One word past the end fails at once with NXM,
...
//                 fails at once with the NXM bit, as muir's
...
// and `dev_ack` and `fb_sel` must be gates on a held match.  A register is
// asked in the one tick after the grant (`quux_mem_port.sv`), the first
// the match has the grant's address, and its mode bit is written then.
//
...
// not there failing, against muir's
