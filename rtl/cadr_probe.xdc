...
#
# READ ONLY WHEN THE PROBE IS IN THE DESIGN.  `vivado/bitstream.tcl` reads
...
# an evening to one.
#
# WHY DRCK NEEDS A CLOCK AT ALL.  `rtl/cadr_probe.sv` clocks its shift
...
# samples it needs the exception `rtl/cadr_machine.xdc` gives the machine's own
...
# split is `rtl/cadr_probe.sv`'s: its inputs stand still from one microcycle
